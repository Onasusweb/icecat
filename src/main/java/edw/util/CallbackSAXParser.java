...
public class CallbackSAXParser extends DefaultHandler {
	...
	/**
	 * @param callback
	 *            The object that implements the callbacks, annotated with @SaxCallback
	 */
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
		...
		this.attributes.put(getFullPath() + "." + qName, elementAttributes);
		stack.push(qName);
		int cleaned = values.cleanup(getFullPath());

		if (debug && cleaned > 0 && !endElementMap.containsKey(getFullPath())) {
			debug("WARNING: Parsing problem: Path " + getFullPath()
					+ " is not unique. You should add a callback for it!");
		}
		...
		if (endElementMap.containsKey(currentPath) || endElementMap.containsKey("*")) {
			try {
				...
				if (method == null)
					...
					debug("Called " + method.getName());

				method.invoke(callback, currentPath, values, this.attributes.get(currentPath));
			} catch (Exception e) {
				exceptionList.add(e);
			}
		}
	...
	public class Values {
		...
		/**
		 * Adds an object to the map Important! Lists of the same element (like: <a>bla</a><a>bla2</a>...) are squashed,
